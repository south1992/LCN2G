...
lc = function(D,M,Z){
  ...
  D_z  = matrix(0,ncol = length(Z),nrow = length(Z))
  for (i in 1:length(Z)){
    for (j in 1:length(Z)){
      D_z[i,j] = Z[i] - Z[j]
    }
...
```{r}
res = list()
TEST = matrix(0,nrow = 10,ncol = choose(10,2))
for(i in 1:10){
  x1 = rnorm(20)
  x2 = rnorm(20)
  x3 = rnorm(20)
  ...
  z = exp(-(x1^2+x10^2)) + 0.2*rnorm(20)
  z = scale(z)
  D = data.frame(x1 = x1,x2 = x2,x3 = rnorm(20),x4 = rnorm(20),x5 = rnorm(20),x6 = rnorm(20),x7 = rnorm(20),x8 = rnorm(20),x9 = x9,x10 = x10)
  D = scale(D)

res[[i]] = lc_exhaust(D,z,k = 2,type = 1)
TEST[i,] = res[[i]]$lc
}
...
```{r}
lc_test(D = data.frame(x = D[,res$searchspace[1,45]],y = D[,res$searchspace[2,45]]),M = diag(2),Z= Z)
```
```{r}
temp_lc = c()
for (i in 1:1000){
...
simu = function(p,n,r,simutype = 1,k = 2,q = 50){
#p = 9
#n = 100
#r = 0.1
...
```{r}
n_list = c("n=100","n=50","n=20","n=100","n=50","n=20")
p_list = c("p=9","p=9","p=9","p=19","p=19","p=19")
Fig2_1 = data.frame(mean = 0,sd = 0,name = "",n = "n=0",p = "p=0")
for (k in 1:6){
  Temp = as.data.frame(R[[k]])
  Temp = Temp[order(Temp[,1],decreasing = F),]
  Temp$name = factor(rownames(Temp),levels = rownames(Temp))
